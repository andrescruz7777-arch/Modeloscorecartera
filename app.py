...
def train_binary_model(df, target_col, num_cols, cat_cols, model_kind):
    ...
    elif model_kind == "xgb" and XGBoost:
        model = XGBClassifier(
    ...
    if uploaded:
        ...
        with st.spinner("Entrenando modelos recalibrados..."):
            ...
            if 'TIENE_GESTION' in df:
                ...
                aucs['Contacto']=auc1; df=add_pred(df,m1,*f1,'P_contacto')
...
with tab2:
    if "df_resultado" not in st.session_state:
        ...
        st.subheader("Distribución de submodelos")
        ...
        st.subheader("Promedio de SCORE_FINAL por CATEGORIA")
        st.bar_chart(df.groupby("CATEGORIA")["SCORE_FINAL"].mean())
